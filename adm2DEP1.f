      subroutine adm2DEP1(n,t0,tf,tm,y0,yout,nit,dff)
...
c-------------Method Runge-Kutta--------------
      do 12, l=2,4
      call eqnjac2DCP1(n,ti(l-1),y1,k1,dff)
      do 13, i=1,nn
      dh(l-1,i)=k1(i)
   13 continue
      do 45, i=1,nn
c     yy(2,i)=yy(1,i)+h*ydh1(i)
      ...
      y2(i)=yy(l,i)
   45 continue
      call eqnjac2DCP1(n,ti(l-1)+h*0.5,y2,k2,dff)
      do 145, i=1,nn
      yy(l,i)=yy(l-1,i)+0.5*h*k2(i)
      y2(i)=yy(l,i)
  145 continue
      call eqnjac2DCP1(n,ti(l-1)+h*0.5,y2,k3,dff)
      do 146, i=1,nn
      yy(l,i)=yy(l-1,i)+k3(i)*h
      y2(i)=yy(l,i)
  146 continue
      call eqnjac2DCP1(n,ti(l),y2,k4,dff)
      do 147, i=1,nn
      yy(l,i)=yy(l-1,i)+1/6.*h*(k1(i)+2*k2(i)+2*k3(i)+k4(i)) 
      y1(i)=yy(l,i)
  147 continue
   12 continue
      call eqnjac2DCP1(n,ti(4),y1,k1,dff)
      do 77, i=1,nn
      dh(4,i)=k1(i)
   77 continue
      do 14, i=1,nn
...
cc----------A-B Four-Step Explicit Method--------------------
     ...
     :*ydh1(i))/24.
      y5(i)=yy(5,i)
   48 continue
      call eqnjac2DCP1(n,ti(5),y5,ydh5,dff)
...
c---------A-B Six Step Explicit Method-----------------------
     ...
     1-7298.*ydh3(i)+2877.*ydh2(i)-475.*ydh1(i))/1440.
      y7(i)=yy(7,i)
   51 continue
      call eqnjac2DCP1(n,ti(7),y7,ydh7,dff)
...
c     close(unit=12)
  117 format(1x,e13.7/)
      ...
      subroutine eqnjac2DCP1(n,t,x,yprime,df)
      integer n,l
      double precision B,F,E,alpha,yprime(2*n),
     :J(n,n),x(2*n),sum,t,df,yp(n)
      external impampl
c
      call impampl(F,B,E,alpha)
c
      J(1,1)=(1+0.5*B)*x(1)*x(2)
      J(1,2)=0.5*(1+0.5*B)*(x(1)**2+3*x(2)**2)
      J(1,3)=0.
      J(1,4)=1.
      J(1,5)=0.
      J(1,6)=0.
      J(2,1)=-0.5*(1+0.5*B)*(3*x(1)**2+x(2)**2)
      J(2,2)=-(1+0.5*B)*x(1)*x(2)
      J(2,3)=0.
      J(2,4)=0.
      J(2,5)=1.
      J(2,6)=0.
      ...
      J(3,5)=0.
      J(3,6)=0.
      J(4,1)=-(2*x(6)-(1+0.5*B)*(2*x(1)*x(5)-x(2)*x(4))-
     :(1+0.5*B)*x(1)*x(5)+6*F*x(1)**2+
     :24./32.*B**2*x(1)**2*(x(1)**2+x(2)**2)+
     :6./32.*B**2*(x(1)**2+x(2)**2)**2
     :+2*(alpha-1)*F*(1/2*(3*x(1)**2-x(2)**2)*dsin(x(3))
     :+2*x(1)*x(2)*dcos(x(3)))*dsin(x(3))+(alpha-1)*F*2
     :*x(1)*(x(1)*dsin(x(3))+x(2)*dcos(x(3)))*dsin(x(3))+
     :(alpha-1)*F*(x(1)**2+x(2)**2)*(dsin(x(3)))**2)
c    
      J(4,3)=-(2*(alpha-1)*F*x(1)*((x(1)**2-x(2)**2)*dsin(x(3))
     :*dcos(x(3))+x(1)*x(2)*(
     :dcos(x(3))**2-dsin(x(3))**2))+(alpha-1)*F*
     :(x(1)**2+x(2)**2)*(2*x(1)*dsin(x(3))*
     :dcos(x(3))+x(2)*(dcos(x(3))**2-dsin(x(3))**2)))
      J(4,2)=-((1+0.5*B)*x(1)*x(4)-(1+0.5*B)*x(2)*x(5)+
     :24./32.*B**2*x(1)*x(2)*(x(1)**2+x(2)**2)
     :+2*(alpha-1)*F*x(1)*(-x(2)*dsin(x(3))
     :+x(1)*dcos(x(3)))*dsin(x(3))+2*(alpha-1)*F*
     :x(2)*(x(1)*dsin(x(3))+x(2)*
     :dcos(x(3)))*dsin(x(3))+(alpha-1)*F*(x(1)**2+
     :x(2)**2)*dcos(x(3))*dsin(x(3)))
      J(4,4)=-((1+0.5*B)*x(1)*x(2))
      J(4,5)=-(-(1+0.5*B)*x(1)**2-0.5*(1+0.5*B)*(x(1)**2+x(2)**2))
      J(4,6)=-2*x(1)
      J(5,1)=-(-(1+0.5*B)*x(2)*x(5)+(1+0.5*B)*x(1)*x(4)+
     :24./32.*B**2*x(2)*x(1)*(x(1)**2+x(2)**2)
     :+2*(alpha-1)*F*x(2)*(x(1)*dsin(x(3))
     :+x(2)*dcos(x(3)))*dsin(x(3))+2*(alpha-1)*F*
     :x(1)*(-x(2)*dsin(x(3))+x(1)*
     :dcos(x(3)))*dsin(x(3))+(alpha-1)*F*(x(1)**2+
     :x(2)**2)*dcos(x(3))*dsin(x(3)))
      J(5,2)=-(2*x(6)-(1+0.5*B)*(x(1)*x(5)-2*x(2)*x(4))+
     :(1+0.5*B)*x(2)*x(4)-6.*F*x(2)**2+
     :6./32.*B**2*(x(1)**2+x(2)**2)**2+24./32.*B**2*x(2)**2*
     :(x(1)**2+x(2)**2)
     :+2*(alpha-1)*F*(1/2*(x(1)**2-3*x(2)**2)*
     :dsin(x(3))+2*x(1)*x(2)*dcos(x(3)))
     :*dsin(x(3))+
     :2*(alpha-1)*F*x(2)*(-x(2)*dsin(x(3))+
     :x(1)*dcos(x(3)))*dsin(x(3))-(alpha-1)*F*
     :(x(1)**2+x(2)**2)*(dsin(x(3)))**2)
      J(5,3)=-(2*(alpha-1)*F*x(2)*((x(1)**2-x(2)**2)*dsin(x(3))
     :*dcos(x(3))+x(1)*x(2)*
     :(dcos(x(3))**2-dsin(x(3))**2))+(alpha-1)*F*
     :(x(1)**2+x(2)**2)*(-2*x(2)*
     :dsin(x(3))*dcos(x(3))+x(1)*(dcos(x(3))**2-dsin(x(3))**2)))
      J(5,4)=-((1+0.5*B)*x(2)**2+0.5*(1+0.5*B)*(x(1)**2+x(2)**2))
      J(5,6)=-2*x(2)
      J(5,5)=(1+0.5*B)*x(2)*x(1)
      J(6,1)=-(alpha-1)*2*x(1)*F*((x(1)**2-x(2)**2)*dsin(x(3))
     :*dcos(x(3))+x(1)*x(2)*
     :(dcos(x(3))**2-dsin(x(3))**2))-(alpha-1)*F*
     :(x(1)**2+x(2)**2)*(2*x(1)*dsin(x(3))*
     :dcos(x(3))+x(2)*(dcos(x(3))**2-dsin(x(3))**2))
      J(6,2)=-2*(alpha-1)*F*x(2)*((x(1)**2-x(2)**2)*dsin(x(3))
     :*dcos(x(3))+x(1)*x(2)*(
     :dcos(x(3))**2-dsin(x(3))**2))-(alpha-1)*F*
     :(x(1)**2+x(2)**2)*(-2*x(2)*dsin(x(3))*
     :dcos(x(3))+x(1)*(dcos(x(3))**2-dsin(x(3))**2))
      J(6,3)=-((alpha-1)*F*(x(1)**2+x(2)**2)*
     :((x(1)**2-x(2)**2)*(dcos(x(3))**2
     :-dsin(x(3))**2)-4*
     :x(1)*x(2)*dcos(x(3))*dsin(x(3))))
      J(6,4)=0.
      J(6,5)=0.
      J(6,6)=0.
